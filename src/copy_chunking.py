...
class Chunk:
    ...
    title: str          # tiêu đề của chunk
    content: str        # nội dung của Chunk
    parent_id: Optional[str] = None # cha của nó là ai
    children_ids: list = field(default_factory=list)    # danh sách con của nó
    embedding: Optional[list] = None    # vector đại diện cho nội dung
...
class HierarchicalParser:
    ...
    def parse(self, text: str) -> dict[str, Chunk]:
        """Phân tích Markdown document thành cây phân cấp chunks.

        Args:
            text (str): Văn bản cần phân tích.

        Returns:
            dict[str, Chunk]: Trả về dict với danh sách các key kiểu str và value kiểu Chunk
        """
        chunks: dict[str, Chunk] = {}  # định nghĩa dict trả về rỗng
        lines = text.split("\n")
